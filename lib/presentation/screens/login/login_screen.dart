                                    ...
                                    child: TextField(
                                      ...
                                      onSubmitted: (value) async {
                                        if (_tel_email != "" && _contrasena != "") {
                                          bool isValid = EmailValidator.validate(_tel_email);
                                          if (isValid) {
...
// ignore: non_constant_identifier_names
Future<String> _check(email, pass) async {
  try {
    ...
    final response = await http.post(
      Uri(
        scheme: https,
        host: host,
        path: '/seg_usuario/app/login/',

        // scheme: https,
        // host: host,
        // path: '$path/seg_usuario/app/login/',
      ),
      body: data,
    ...
    if (response.statusCode == 200) {
      ...
      final bool isSuccess = jsonResponse['response'];
      final Map<String, dynamic> result = jsonResponse['result'];
      if (isSuccess == true) {
        return "Datos Correctos,${result['id']},${result['nombre']},${result['nombre_encargado']},${result['telefono']},${result['email']},${result['tipo']}";      
      }else{
...
Future<void> showResultDialog(BuildContext context, String result, String telEmail, String pass) async {
  ...
  if (result == 'Error, verificar conexión a Internet') {
    ...
    // log(splitted[2].toString());
    // log(splitted[3].toString());
    // log(splitted[4].toString());
    // log(splitted[5].toString());
    // log(splitted[6].toString());
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString('id', splitted[1].toString());
    prefs.setString('nombre', splitted[2].toString());
    prefs.setString('nombre_encargado', splitted[3].toString());
    prefs.setString('telefono', splitted[4].toString());
    prefs.setString('email', splitted[5].toString());
    prefs.setString('tipo', splitted[6].toString());
    ...
    if(splitted[6] == '3'){
      ...
      Navigator.of(context).push(
        PageRouteBuilder(
          ...
          opaque: false,
          pageBuilder: (_, __, ___) => const PremioScreen(),
