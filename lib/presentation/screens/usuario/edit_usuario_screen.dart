...
class EditarUsuarioScreen extends StatefulWidget {
  static const String routeName = 'editar_usuario';
...
class _EditarUsuarioScreenState extends State<EditarUsuarioScreen>
  ...
  void initState() {
    ...
    // log(args['nombre'].toString());
    // log(args['nombre_encargado'].toString());
    // log(args['tipo'].toString());
    // log(args['email'].toString());
    // log(args['telefono'].toString());

    setState(() {
      textController1.text = args['nombre'] ?? '';
      nombre = args['nombre'].toString();

      textController2.text = args['nombre_encargado'] ?? '';
      nombreEncargado = args['nombre_encargado'].toString();

      _rolSeleccionado = args['tipo']=="1" 
        ? "Administrador"
        : args['tipo']=="2"
          ? "Stand"
          : "Premios";
      tipo = _rolSeleccionado=="Administrador" 
        ? 1
        : _rolSeleccionado=="Stand" 
          ? 2
          : 3;

      textController3.text = args['email'] ?? '';
      email = args['email'].toString();

      textController4.text = args['telefono'] ?? '';
      telefono = args['telefono'].toString();
    });
  });
  }
          ...
          return PopScope(
            ...
            child: Container(
              ...
              child: RouteAwareWidget(
                screenName: "editar_usuario",
                child: Scaffold(
                  key: _scaffoldKey,
                  backgroundColor: Colors.white.withOpacity(1),
                  drawer: SideMenu(userapp: _userapp ?? "0", tipoapp: _tipoapp ?? "0", idapp: args['idapp'].toString()),
                  appBar: AppBar(
                    title: const Text(nameEditarUsuario),
                    elevation: 1,
                    shadowColor: myColor,
                                  ...
                                  onSubmitted: (value) {
                                    if (nombre != "") {
                                      ...
                                      if (isValid) {
                                          ...
                                          // log(telefono);
                                          // log(password);
                                          showProgress(context, args['id'].toString(), nombre, nombreEncargado, tipo.toString(), email, telefono, password);
                                      } else {
                                        awesomeTopSnackbar(
                                          context,
  ...
  //showProgress
  showProgress(BuildContext context, String id, String nombre, String nombreEncargado, String tipo, String email, String telefono, String password) async {
    var result = await showDialog(
      context: context,
      builder: (context) => FutureProgressDialog(_check(id, nombre, nombreEncargado, tipo, email, telefono, password)),
    );
  ...
  Future<String> _check(id, nombre, nombreEncargado, tipo, email, telefono, password) async {
    try {
      var data = password != ""
        ? {
            "nombre": nombre,
      ...
      final response = await http.post(
          Uri(
            scheme: https,
            host: host,
            path: "/seg_usuario/edit/$id",

            // scheme: https,
            // host: host,
            // path: '$path/seg_usuario/edit/$id',
          ),
          body: data,
      ...
      if (response.statusCode == 200) {
        String body3 = utf8.decode(response.bodyBytes);
        var jsonData = jsonDecode(body3);
        if (jsonData['message'] == 'Editado exitosamente') {
  ...
  Future<void> showResultDialog(BuildContext context, String result) async {
    if (result == 'Editado exitosamente') {
      final args = ModalRoute.of(context)!.settings.arguments as Map<String, dynamic>;
      navigatorKey.currentState?.pushNamedAndRemoveUntil(
        UsuarioScreen.routeName,
        (Route<dynamic> route) => false,
